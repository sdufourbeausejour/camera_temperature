...
for i, year in enumerate([2015,2016,2017]):
    ...
    #interp difference
    f_interp_one_period = interp1d(v_date2num(yearly_mean_diff.index), smooth_diff, 'cubic')
    axes[i+3].set_xlim(xmin=xmin,xmax=xmax)   # limit for xaxis
    axes[i+3].plot_date(v_date2num(yearly_mean_diff.index),f_interp_one_period(v_date2num(yearly_mean_diff.index)), linewidth=1 ,marker="",linestyle="-",color="g")
