...
for i, year in enumerate([2015,2016,2017]):
    ...
    axes[i+3].plot_date(yearly_mean_diff.index,smooth_diff, linewidth=1 ,marker="",linestyle="-",color="k")
    mu = mpl.dates.date2num(datetime.datetime(year+1,5,1))
    sigma = 50
    gaussian = stats.norm.pdf(v_date2num(yearly_mean_diff.index), mu, sigma)
