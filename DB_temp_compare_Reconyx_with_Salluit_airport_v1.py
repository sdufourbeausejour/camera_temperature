...
data["EC"] = pd.read_csv("data/EC/S_EC_temperatures_assembled_2.txt", header=13)
...
for i, year in enumerate([2015,2016,2017]):
    ...
    axes[i+3].plot_date(v_date2num(intersection["DB"].index),intersection["DB"].values - intersection["EC"].values, linewidth=0.4 ,marker=" ",linestyle="-",color="k")
    axes[i+3].axhline(y=0, ls="-", c="k", linewidth=0.5)
    # Mean diff for that year
    yearly_mean_diff_ind = (intersection["DB"].index > mpl.dates.num2date(xmin)) & (intersection["DB"].index <= mpl.dates.num2date(xmax))
    yearly_mean_diff = intersection["DB"][yearly_mean_diff_ind] - intersection["EC"][yearly_mean_diff_ind]
    mean_diff = np.mean(yearly_mean_diff)
    axes[i+3].axhline(y=mean_diff, ls="--", c="b", linewidth=0.5)
    axes[i+3].annotate(r"$\Delta T_{mean}$ = "+"{:.1f}".format(mean_diff)+r" $^\circ$C", xy=(0.04,0.75), xycoords="axes fraction",fontsize=12,color="b")
    axes[i+3].annotate(str(year)+"-"+str(year+1), xy=(0.04,0.875), xycoords="axes fraction",fontsize=12,color="k")
    axes[i+3].set_ylabel(r"$\Delta T_{DB - S}$ ($^\circ$C)")
