...
# daily mean
daily_mean = {}
for camera in ["Nord_268","Hearn_200"]:
...
intersection = pd.concat([daily_mean["Nord_268"], daily_mean["Hearn_200"]], axis=1, join='inner')
intersection.columns = ["Nord_268","Hearn_200"]
...
axes = axes.flatten()
fig.suptitle("Comparing Quaqtaq Reconyx Temperature Measurements - Different Location", fontsize=12)
for i, year in enumerate([2017]):
    ...
    axes[i].plot_date(daily_mean["Hearn_200"].index, daily_mean["Hearn_200"], linewidth=0.4,marker=" ",linestyle="-",color="r")
    axes[i].annotate(r"Reconyx Nord 268", xy=(0.6,0.20), xycoords="axes fraction",fontsize=12,color="k")
    axes[i].annotate(r"Reconyx Hearn 200", xy=(0.6,0.10), xycoords="axes fraction",fontsize=12,color="r")
    # Plot difference
    axes[i+1].set_xlim(xmin=xmin,xmax=xmax)   # limit for xaxis
    axes[i+1].plot_date(v_date2num(intersection.index),intersection["Nord_268"].values - intersection["Hearn_200"].values, linewidth=0.4 ,marker=" ",linestyle="-",color="k")
    ...
    yearly_mean_diff = intersection["Nord_268"][yearly_mean_diff_ind] - intersection["Hearn_200"][yearly_mean_diff_ind]
    mean_diff = np.mean(yearly_mean_diff)
    axes[i+1].axhline(y=mean_diff, ls="--", c="b", linewidth=0.5)
    axes[i+1].annotate(r"$\Delta T_{mean}$ = "+str(0.1*np.round(mean_diff*10))+r" $^\circ$C", xy=(0.04,0.1), xycoords="axes fraction",fontsize=12,color="b")
    axes[i+1].annotate(str(year)+"-"+str(year+1), xy=(0.04,0.875), xycoords="axes fraction",fontsize=12,color="k")
    axes[i+1].set_ylabel(r"$\Delta T_{268 - 200}$ ($^\circ$C)")
    axes[i+1].yaxis.set_ticks(np.arange(-10,10,0.5), minor=True)
