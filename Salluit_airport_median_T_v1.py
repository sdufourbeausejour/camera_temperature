...
for i, year in enumerate([2015,2016,2017]):
    ...
    axes[i].yaxis.set_ticks(np.arange(-45,30,15))
    axes[i].yaxis.set_ticks(np.arange(-45,25,5), minor=True)
    axes[i].set_ylim(-40,20)
    ...
    # Group by month, median, center at beginning of month using label
    monthly["med"] = yearly_T.resample("M", label="left").median()


    axes[i].set_ylabel(r"Median T ($^\circ$C)")
    ...
    v = monthly["med"].loc[monthly["med"] <= 0]
    axes[i].bar(v.index, v, width=10, color="teal",bottom=None, align='center', data=None)
    for d,n_f in zip(v.index, v):
        axes[i].annotate("{:.0f}".format(n_f), xy=(d, n_f-3), xycoords="data", fontsize=6, color="teal", ha="center")

    v = monthly["med"].loc[monthly["med"] > 0]
    axes[i].bar(v.index, v, width=10, color="orange", bottom=None, align='center', data=None)
    for d,n_t in zip(v.index, v):
        axes[i].annotate("{:.0f}".format(n_t), xy=(d, n_t+1), xycoords="data", fontsize=6, color="orange", ha="center")
