...
intersection = pd.concat([daily_mean["Hearn_200"], daily_mean["EC"]], axis=1, join='inner')
...
axes = axes.flatten()
fig.suptitle("Comparing Quaqtaq Reconyx Hearn Temperature with Airport", fontsize=12)
for i, year in enumerate([2017]):
    ...
    # Plot each series
    axes[i].plot_date(daily_mean["Hearn_200"].index, daily_mean["Hearn_200"], linewidth=1,marker=" ",linestyle="-",color="k")
    ...
    axes[i+1].annotate(r"$\Delta T_{mean}$ = "+str(0.1*np.round(mean_diff*10))+r" $^\circ$C", xy=(0.04,0.75), xycoords="axes fraction",fontsize=12,color="b")
