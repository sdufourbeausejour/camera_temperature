...
intersection = pd.concat([hourly["Nord_268"], hourly["EC"]], axis=1, join='inner')
...
axes = axes.flatten()
fig.suptitle("Comparing Quaqtaq Reconyx Nord Temperature Measurements with Airport - Hourly", fontsize=12)
for i, year in enumerate([2015,2016,2017]):
    ...
    # Plot each series
    axes[i].plot_date(hourly["Nord_268"].index, hourly["Nord_268"], linewidth=1,marker=" ",linestyle="-",color="k")
